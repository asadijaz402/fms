...
import DeleteTyresBookingDialog from "./Dialogs/DeleteTyresBookingDialog";
...
export default function TyresBooking() {
  const ActionButtons = ({ row }) => {
    return (
      <Box display="flex">
        <Box>
          <TyresBookingDialog rowId={row.id} status={row.status} />
        </Box>
        {row.status !== "Completed" && (
          <Box ml={1}>
            <DeleteTyresBookingDialog rowId={row.id} />
          </Box>
        )}
      </Box>
    );
  ...
  return (
    <DynamicDataTable
      title="Vehicles booked for Tyre service (Live vehicles only)"
      urlLink="vehicle_accessories/tyres/list/?"
      columnsCustom={columns}
      addNew={[<TyresBookingDialog />]}
    />
  );
}
